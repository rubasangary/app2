...
@section('content')
        ...
        <h4 class="font-semibold mt-4 mb-4 text-gray-700 dark:text-gray-300"><i class="fa-solid fa-drum fa-lg mr-3" style="color: #0a5ae6;"></i><span>ஊர் புதினம்</span></h4>
                ...
                <div class="w-full lg:w-1/2 xl:w-2/3 2xl:w-3/4 rounded-lg h-full">
                        ...
                        <div class="block rounded-lg border border-gray-300 bg-transparent dark:border-gray-700 bg-white dark:bg-gray-800">
                            ...
                            <div class="border-t-2 border-gray-300 px-6 py-3 text-gray-700 dark:border-gray-700 dark:text-gray-300 flex justify-between">
                                ...
                                <div>
                                    @auth
                                        @if (Auth::id() !== $forum->forumPostedBy->id)
                                            @if (Auth::user()->isFollowing($forum->forumPostedBy))
                                                <form action="{{ route('user.unfollow', $forum->forumPostedBy) }}" method="POST">
                                                    @csrf
                                                    <button type="submit">
                                                        Unfollow
                                                    </button>
                                            ...
                                            @else
                                                <form action="{{ route('user.follow', $forum->forumPostedBy) }}" method="POST">
                                                    @csrf
                                                    <button type="submit">
                                                        Follow
                                                    </button>
                                        ...
                                        {{-- <div>
                                            ...
                                            @guest
                                                <p>
                                                    ...
                                                    <a href="{{ route('login', ['redirect' => Request::fullUrl()]) }}">Login</a>
                                                </p>
                                            @endguest
                                        </div> --}}

                                <div>
                                    <livewire:like-button :forum="$forum" />
                                </div>
