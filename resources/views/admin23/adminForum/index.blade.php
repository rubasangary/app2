...
<div class="w-full mt-6">
    ...
    <div class="flex justify-between">
        <p class="text-xl pb-3 flex items-center text-gray-700 font-semibold">
            <i class="fas fa-list mr-3"></i>Manage Forum
        </p>
    </div>
                ...
                <div class="overflow-hidden">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b bg-neutral-100 dark:border-neutral-500 dark:bg-neutral-700">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">{{$forum->forumPostedBy->id}}</td>

                            <td class="whitespace-nowrap px-6 py-4 font-medium">
                                @if ($forum ->forumPostedBy->profile_photo_url)
                                        <img src="{{ $forum ->forumPostedBy->profile_photo_url }}" width="60px" height="50px">
                                    @else
                                        <img src="/img/man.jpg" width="120">
                                    @endif
                                    {{ $forum->forumPostedBy->name }}
                            </td>

                            <td class="whitespace-nowrap px-6 py-4 truncate">{{ Illuminate\Support\Str::limit($forum->content, 60) }}</td>

                            <td class="whitespace-nowrap px-6 py-4">{{$forum->created_at->format('Y-m-d')}}</td>

                            <td class="whitespace-nowrap px-6 py-4 ">
                                <a class="font-semibold text-sm bg-green-700 hover:bg-green-500 py-1 px-2 text-white rounded" target="_blank"
                                href="{{ route('forum.show', $forum->id) }}">View</a>
                            </td>
